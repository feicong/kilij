...
#define CRYPTOUTILS_IMPLEMENTATION
#include "CryptoUtils.h"
#undef CRYPTOUTILS_IMPLEMENTATION
...
static cl::opt<uint64_t>
    ObfSeed("obf-seed", cl::init(0),
            cl::desc("Seed for obfuscation randomness (0 = random)"));

// Expand -obf-seed (a single u64) into a 128-bit AES key.  splitmix64 is a
// bijective integer hash -- good enough for key derivation here since the
// security requirement is reproducibility, not unpredictability.
...
static std::string obfSeedToHex(uint64_t seed) {
...
namespace llvm {
uint64_t getObfuscationSeed() { return ObfSeed; }

std::string getObfuscationSeedHex() {
  if (ObfSeed == 0)
    return "";
  return obfSeedToHex(ObfSeed);
}
} // namespace llvm
...
CryptoUtils::CryptoUtils() {
  ...
  if (!AesSeed.empty()) {
    if (!prng_seed(AesSeed.c_str()))
      report_fatal_error("obf: failed to seed AES-CTR PRNG");
  } else if (ObfSeed != 0) {
    std::string hex = obfSeedToHex(ObfSeed);
    if (!prng_seed(hex.c_str()))
      report_fatal_error("obf: failed to seed AES-CTR PRNG");
  }
}
...
bool CryptoUtils::prng_seed(const std::string _seed) {
  ...
  for (unsigned int j = 0; i < _seed.size(); i += 2, j++) {
    ...
    s[j] = (unsigned char)(int)strtol(byte.c_str(), nullptr, 16);
  }
...
// CTR mode: encrypt an incrementing counter to fill the pool.  Each 16-byte
// AES block produces 16 bytes of output, giving PoolSize/16 blocks per refill.
void CryptoUtils::populate_pool() {

  statsPopulate++;

  for (int i = 0; i < static_cast<int>(PoolSize); i += 16) {
    inc_ctr();
...
bool CryptoUtils::prng_seed() {
  ...
  // Seed from OS RNG when no explicit seed is provided.
...
void CryptoUtils::get_bytes(char *buffer, const int len) {
  ...
  if (len > 0) {
    ...
    do {
      uint32_t remaining = static_cast<uint32_t>(len - sofar);
      if (idx + remaining >= PoolSize) {
        // We don't have enough bytes ready in the pool,
        // so let's use the available ones and repopulate !
        available = static_cast<int>(PoolSize - idx);
        memcpy(buffer + sofar, pool + idx, available);
        sofar += available;
        populate_pool();
      } else {
        memcpy(buffer + sofar, pool + idx, remaining);
        idx += remaining;
...
uint32_t CryptoUtils::get_range(const uint32_t max) {
  ...
  if (max == 0) {
    ...
    do {
      r = get_uint32_t() & static_cast<uint32_t>(mask);
    } while (r >= max);
