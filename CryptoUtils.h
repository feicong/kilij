...
class CryptoUtils;
// Global PRNG state is shared; deterministic builds depend on call ordering.
extern ManagedStatic<CryptoUtils> cryptoutils;

#ifdef CRYPTOUTILS_IMPLEMENTATION
...
#if !defined(_WIN32) && !defined(UNALIGNED)
#define UNALIGNED
#endif
...
#define RORc(x, y)                                                             \
  (((((unsigned long)(x) & 0xFFFFFFFFUL) >> (unsigned long)((y) & 31)) |       \
   ...
   0xFFFFFFFFUL)
#endif // CRYPTOUTILS_IMPLEMENTATION

// AES-128 in CTR mode used as a CSPRNG.  Seeded from the OS RNG (or a
// user-provided hex seed for reproducible builds).  The 128KB pool is
// refilled by encrypting successive counter values; consumers draw bytes
// sequentially until the pool is exhausted, then it repopulates.
class CryptoUtils {
public:
  CryptoUtils();
  ~CryptoUtils();
...
private:
  // 128KB pool balances refill frequency vs memory; each refill encrypts
  // PoolSize/16 AES blocks, so larger pools amortize the key-schedule cost.
  static constexpr uint32_t PoolSize = (0x1u << 17);
...
uint64_t getObfuscationSeed();
// Hex form is stored in metadata so builds can be reproduced with a fixed seed.
std::string getObfuscationSeedHex();
}
